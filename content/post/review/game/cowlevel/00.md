...
目前最新版本为IIDX 30 Resident，中文音游圈通常可以找到的版本为**最新版本N-1**。
...
首先，你需要自己寻找HDD的资源，理科论坛（bemani.cc）更容易获得你想要的东西。现在理科论坛已经开放注册（需要绑定手机号）并且限制国内IP访问。

当然，你也可以直接去墙外寻找资源，比如[emuline论坛](https://www.emuline.org/)，语言默认法文，可以在右上角调整为英文。甚至连大家更熟悉的[nyaa种子站](https://nyaa.si/)也能直接搜出相关资源，除了IIDX29。
...
离线服务器有很多，但我只推荐Asphyxia-core。这是最新的模组化地服务器，由内核与插件共同组成。插件支持大量Bemani音游，而且没有功能阉割，甚至可以通过各类虚拟局域网工具让大家一起玩。[内核下载地址](https://asphyxia-core.github.io/)，[IIDX插件下载地址](https://github.com/rumatsuki/iidx-plugin/tree/master)。IIDX插件需要简单的修改才能支持IIDX30，如果你有Typescript代码知识，很快就能搞定。

![img](https://asphyxia-core.github.io/img/core-logo.png)

开发者为了规避只提供了Linux和Windows版本可执行文件，另外也提供了额外的ARM架构版本，理论上可以在绝大多数操作系统中运行。为了规避Konami法务部铁拳而将插件部分交给开源社群自己编写，支持最新的IIDX 30。

使用自己架设的服务器虽然可以保存数据，但与此同时整个服务器只有你一个玩家，所以也就没法用排行榜之类的功能。
...
**离线服务器**：由于IIDX默认会访问服务器检测，不联网游戏则无法运行，所以需要一个伪造的本地服务器来骗过检测。其实就是代理服务器，伪造e-amusement服务器正在维护中的离线效果。
